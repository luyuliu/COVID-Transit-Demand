...
for each_system in rl_system:
    ...
    p_guess = (int(np.median(x)), 0, 100, 0.5)
    # if system_name == "CATA":
    #     continue
    ...
    plt.xlabel('x: days since 2/15/2020')
    plt.ylabel('y: transit demand decrease (%)')
    plt.grid(True)
    plt.title("Scatter Point Plot of {} Transit Demand Decrease".format(system_name), fontsize=16)
...
#     plt.ylabel('y: transit demand decrease (%)')
#     plt.grid(True)
#     plt.title("Scatter Point Plot of Transit Demand Decrease", fontsize=16)
