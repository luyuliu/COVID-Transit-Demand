...
for each_system in rl_system:
    ...
    try:
        ...
        print(state)
        continue
    rl_case = list(col_case.find({"state": the_state_abbr, "county": county + " County"}))
    if the_state_abbr == "DC":
        rl_case = list(col_case.find({"state": the_state_abbr, "county": county}))
    if rl_case != []:
        rl_case = rl_case[0]
        col_system.update_one({"_id": _id}, {"$set":{"county_FIPS": rl_case["county_FIPS"], "state_FIPS": rl_case["state_FIPS"]}})
    else:
        print(county)